...
export default function StockIncomingScreen() {
  const items = useInventoryStore((s) => s.items)
  const incoming = useInventoryStore((s) => s.incoming)
  const addIncoming = useInventoryStore((s) => s.addIncoming)
  const [selected, setSelected] = useState(items?.[0]?.id || null)
  const [qty, setQty] = useState(10)

  const submit = () => {
    if (!selected || !qty) return
    addIncoming(selected, qty, 'manual')
  }

  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Incoming Stock</Text>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        ...
        <Pressable onPress={submit} style={styles.submit}><Text style={styles.submitText}>Add</Text></Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  submit: { backgroundColor: '#10b981', paddingVertical: 10, paddingHorizontal: 14, borderRadius: 8 },
